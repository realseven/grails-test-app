...
class TestController {
    ...
    def save(Test testInstance) {
		...
		//run sys cmd to run test
		def date = System.currentTimeMillis()
		//def logPath =  "/var/log/myapp/" + date + ".xml"
		def logPath =  "myAutoTestLogs/" + date + ".xml"
		def runTestCmd = "pybot --output " + logPath + " --log NONE --report NONE scripts/mytest.robot"
		def process = runTestCmd.execute()
		println "execute test case: " + runTestCmd
		println "${process.text}"
		
		//save logPath
		testInstance.logPath = logPath
